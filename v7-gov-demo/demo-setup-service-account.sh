...
SERVICEACCOUNT_RESPONSE=$(curl -X POST -H "Authorization: Bearer $USER_ACCESS_TOKEN" -H "X-Axway-Tenant-Id: $ORG_ID" -H "Accept: application/json" -H "Content-Type: application/json" -d "$SERVICEACCOUNT_CONTENT" https://apicentral.axway.com/api/v1/serviceAccounts)
